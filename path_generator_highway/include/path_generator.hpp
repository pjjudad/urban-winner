...
namespace ns_path_generator {
    ...
    public:
        ...
        fsd_common_msgs::Trajectory getRefTrajectory();  //获取规划的路径
        ...
        void setMidPoint(const fsd_common_msgs::Trajectory &midPoint);
    ...
    private:
        ...
        fsd_common_msgs::Map local_map_;  //不管是Track类里面的map_,还是PathGenerator里面的lacal_map_ 实际传入的都是path_generator_handle订阅的 /map 
        fsd_common_msgs::CarState car_state_;
        fsd_common_msgs::Trajectory midPoint_;
        Trajectory refline_;

        fsd_common_msgs::ControlCommand cmd_;   //在哪里赋值了？
        Track *track_;   //指针指向Autox_Track、Line_Track、Skidpad_Track其中一种 
        //Track是基类 在~path_generator/include/Track/track_base.h连定义
