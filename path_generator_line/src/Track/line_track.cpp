...
namespace ns_path_generator {
   // 在line_track.cpp的构造函数中添加
Line_Track::Line_Track(ros::NodeHandle& nh) {
    trajectory_points_pub_ = nh.advertise<sensor_msgs::PointCloud>("line_trajectory_points", 10);
}
    bool Line_Track::genTraj() {
        const double interval = param_.interval;
        const double desire_vel = param_.desire_vel;
        // 添加日志输出终点坐标 
    ROS_INFO("Using end point in Line_Track: (%.3f, %.3f)", endPoint_.x, endPoint_.y);
        ...
        for (double i = 0; i < distance; i += interval) {  
            ...
            // std::cout<<"tmp_pt.pts.x="<<tmp_pt.pts.x<<"tmp_pt.pts.y="<<tmp_pt.pts.y<<std::endl;
        }
        const size_t max_points = 10000;
    if (interval < 0.01) {
        ROS_ERROR("Interval is too small, may cause memory explosion!");
        return false;
    }
    if (distance / interval > max_points) {
        ROS_ERROR("Too many trajectory points, aborting!");
        return false;
    }
    // 添加点云发布代码
    sensor_msgs::PointCloud point_cloud;
    point_cloud.header.stamp = ros::Time::now();
    point_cloud.header.frame_id = "world"; // 或您需要的坐标系
    ...
    trajectory_points_pub_.publish(point_cloud);
        ROS_INFO("Generated %zu trajectory points.", trajectory_.size());
        return true;
    }
    ...
    bool Line_Track::CalculateTraj(Trajectory &refline) {
     ...
     //   if (trajectory_.empty()) {
      ...
      //  }
        ...
        //  std::cout<< "xheshenzhuangtai\n "<<std::endl;
         std::cout<< "state_.x "<<state_.x<<std::endl;
        ...
        for (int i = 0; i < N; i++) {
            ...
            int next = std::min(index + index_min, static_cast<int>(trajectory_.size()) - 1);
            if (next < 0) next = 0;



            //  std::cout<< "guiji dain geshu ::________"<<std::endl;
          ...
          //  double temp_x, temp_y;   
           ...
           // temp_y = delta_y * cos(psi) - delta_x * sin(psi);  // y cos（） - x sin（）  这个为什么？
            ...
            //tmp.curvature = fabs(trajectory_[next].curvature);  //曲率look_ahead
            tmp.curvature = trajectory_[next].curvature; 
            //现在的x，y和yaw 是考虑了车辆位姿的
            tmp.pts = cv::Point2f(delta_x, delta_y);
            //tmp.yaw = delta_yaw;  //
             tmp.yaw = trajectory_[next].yaw;
